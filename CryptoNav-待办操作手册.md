...
> ### ⚠️ 不要再去断开 Cloudflare 的 Git 集成（2026-09-11 实测推翻原建议）
>
> 这里原来写着"建议去 Cloudflare 后台把 Pages 的 Git 集成关掉，避免构建两遍"。
> **照做会让所有数据更新停止上线。**
>
> 原因是一条 GitHub 的设计：**用仓库自带的 `GITHUB_TOKEN` 推送产生的 push 事件，
> 不会触发其他工作流**（官方行为，用来防止工作流递归触发）。
> 而 5 个数据刷新工作流（chain tokens / safety / airdrops / unlocks / wins）
> **全部**用 `secrets.GITHUB_TOKEN` 推送：
>
> | 谁推的 | 会触发本 workflow 吗 | 谁来构建 |
> |---|---|---|
> | 我 / 你（真人凭据） | ✅ 会 | 本 workflow **＋** Git 集成 = 两遍 |
> | `github-actions[bot]`（数据刷新） | ❌ **不会** | **只有** Git 集成 |
>
> **证据**（近 12 天逐日比对，11/12 天吻合）：
>
> | 日期 | 机器人提交 | 真人提交 | 部署运行数 |
> |---|---|---|---|
> | 9-03 | 7 | 0 | **0** |
...
> | 9-11 | 1 | 8 | 3 |
>
> **所以 Git 集成现在是唯一在构建数据更新的通道。** 一旦断开，价格 / 安全报告 /
> 空投 / 解锁这些数据就再也不会更新到线上 —— 而且**不会有任何报错**，
> 站点看起来一切正常，只是数据一天天变旧。这是最难发现的一类故障。
>
> **二选一，别做第三条：**
>
> - **省事（推荐）**：Git 集成保持不动。想省掉"真人推送构建两遍"，就删掉
>   `deploy-pages.yml`，或去掉它的 `push:` 触发、只留 `workflow_dispatch`
>   （手动部署 + 预览分支用）。代价是失去那 13 项预检和清楚的报错。
> - **要 Wrangler 当唯一通道**：把 5 个数据工作流的 `actions/checkout` 里的
>   `token: ${{ secrets.GITHUB_TOKEN }}` 换成 PAT（fine-grained，
>   `contents: write` + `actions: write`）。这样机器人推送也能触发部署，
>   **此时才可以**断开 Git 集成。
>
> **只断开集成、不改 token = 数据静默停止更新。**
...
# 已上线 · 链接健康巡检（每周二 04:00 UTC 自动跑）

`.github/workflows/check-links.yml`，两个 job，**不需要你做任何事**。

## 它查什么

| job | 查什么 | 首次 CI 实测（2026-09-04） |
|-----|--------|--------------------------|
| `crawl` | 站内链接：110 个 sitemap URL 上提取出的 110 条内链 | 0 死链，跑 31 秒 |
| `outbound` | 站外：63 个 active 项目的官网还活着吗 | 正常 45 / 拦截 18 / **失效 0** / 超时 0 |

## 为什么「拦截 18 个」不算问题

这 18 个（Gate.io、Etherscan、OpenSea 等）部署在 Cloudflare 后面，对数据中心 IP
和脚本 UA 返回 403。**站点是活的，只是不欢迎这个请求**。

脚本刻意把它们单列成 `block`，**不影响退出码**。因为如果把反爬拦截算成失效，
这个 job 就会每周报红，然后被当成噪音忽略 —— 而一旦开始忽略，
某周真正的死链也会一起被淹没。

**只有「确认失效」才让 job 变红**：DNS 解析失败 / 连接被拒 / 404 / 410 /
证书无效 / 重试后仍 5xx。

## 怎么读结果

- **绿灯** = 没事。
- **红灯** = 打开 Annotations 面板，死链会直接列出来（不用翻日志）。
- **黄灯（warning）** = 出现了**新的**跨域名重定向，需要人工判断是官方改名
  还是域名被卖了。

## ⚠️ 6 个已知重定向不要去「修正」

清单在 `scripts/known-redirects.json`，每条都写了日期和理由。

- **5 个是官方改名**（Curve / 1inch / dYdX / Phantom / Arkham），
  不改的理由很反直觉：1inch.com（1999）、phantom.com（1992）、arkm.com（2001）
  都是**二级市场买来的老域名**。把 website 改成它们，longevity 会从
  7.3 / 5.8 / 6.6 年跳到 26.9 / 34.6 / 24.9 年，
  **制造 3 个新的 blur 式评分污染**。
- **Magic Eden 是另一回事**：`magiceden.io → magiceden.us/?gr` 是地理分流
  （`gr` = geo redirect），只有美国 IP 会被送走。本站面向国际用户，**必须保留 .io**。

复核过且落点未变的不再重复告警；**落点变了反而会大声报**（域名又动了，可能被卖了）。
