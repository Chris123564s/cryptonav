 ...
 * Safety verification
 *
 * Generated by scripts/build-safety-report.mjs. Read-only here: pages must
 * never invent a safety fact, only render one that the pipeline proved.
 * ------------------------------------------------------------------ */
import safetyData from '../data/safety.json';
import type { SafetyProject, SafetyReport } from '../types';

export const safety = safetyData as SafetyReport;

export function getSafety(id: string): SafetyProject | null {
  return (safetyData as SafetyReport).projects?.[id] ?? null;
}

/** Projects that have a verification record, highest score first. */
export function getVerifiedProjects(): Array<{ project: ProjectData; safety: SafetyProject }> {
  return projects
    .map((project) => ({ project, safety: getSafety(project.id) }))
    .filter((row): row is { project: ProjectData; safety: SafetyProject } => row.safety !== null)
    .sort((a, b) => {
      if (a.safety.score === null && b.safety.score === null) return a.project.name.localeCompare(b.project.name);
      if (a.safety.score === null) return 1;
      if (b.safety.score === null) return -1;
...
export function gradeColor(grade: string): string {
  return { A: '#00b87a', B: '#3fa9f5', C: '#f5a623', D: '#f5653f', E: '#e0245e' }[grade] ?? '#8b8b9e';
}
